...
class QobuzDownloader:
    def __init__(self):
        ...
        try:
            # Инициализируем клиент и сразу получаем токены, чтобы иметь доступ к app_id
            self.client = QobuzDL()
            self.client.get_tokens()
            logger.info("✅ Клиент Qobuz успешно инициализирован и готов к поиску.")
    ...
    def search_track(self, artist: str, title: str) -> Optional[str]:
        """Ищет трек по артисту и названию через официальный API Qobuz."""
        if not self.client:
        ...
        # Используем app_id, полученный при инициализации клиента
        app_id = getattr(self.client, 'app_id', None)
        if not app_id:
            logger.error("❌ Не удалось получить app_id из клиента Qobuz.")
            return None

        params = { "query": query, "limit": 10, "app_id": app_id }
        ...
        try:
            ...
            if results and results.get('tracks', {}).get('items'):
                # Ищем наиболее релевантный результат, сравнивая и артиста
                for item in results['tracks']['items']:
                    track_title_from_qobuz = item.get('title', '').lower()
                    performer_from_qobuz = item.get('performer', {}).get('name', '').lower()
                    if clean_title.lower() in track_title_from_qobuz and artist.lower() in performer_from_qobuz:
                        track_id = item['id']
                        url = f"https://open.qobuz.com/track/{track_id}"
                        logger.info(f"✅ Найден точный трек на Qobuz: {url}")
                        return url
                
                # Если точного совпадения нет, берем самый первый результат
